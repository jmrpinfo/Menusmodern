...
local options = {
    ...
    -- Restart a thumbnailer that did not produce a new frame after a seek
    watchdog_timeout = 3,

    -- Maximum automatic restarts before waiting for the pointer to leave
    watchdog_retries = 2,

    -- Refresh a still-running helper when previews resume after this many seconds
    restart_after_idle = 15,
...
local last_request_clock
local watchdog_retries = 0
...
local function spawn(time)
    ...
    local this_generation = spawn_generation

    subprocess_id = subprocess(args, true,
        function(success, result)
            -- A watchdog restart can retire an older subprocess while its
            -- asynchronous callback is still pending. Ignore that stale exit.
            if this_generation ~= spawn_generation then return end
            subprocess_id = nil
...
local function stop_thumbnailer(force)
    -- Retire the callback before asking the process to stop; aborting an
    -- asynchronous subprocess can complete its callback immediately.
    spawn_generation = spawn_generation + 1
    if spawned then run("quit") end
    if force and subprocess_id and mp.abort_async_command then
        pcall(mp.abort_async_command, subprocess_id)
    end
    subprocess_id = nil
    spawned = false
    spawn_waiting = false
...
local restart_thumbnailer

local function stop_watchdog()
    if watchdog_timer then watchdog_timer:kill() end
end

local function arm_watchdog()
    if options.watchdog_timeout <= 0 or not show_thumbnail then return end
    if not watchdog_timer then
        watchdog_timer = mp.add_timeout(options.watchdog_timeout, function()
            if restart_thumbnailer then restart_thumbnailer("sem novo quadro") end
        end)
    else
        watchdog_timer:kill()
        watchdog_timer.timeout = options.watchdog_timeout
    end
    watchdog_timer:resume()
end

restart_thumbnailer = function(reason)
    if disabled or not show_thumbnail or last_seek_time == nil then return end
    if watchdog_retries >= math.max(1, options.watchdog_retries) then
        stop_watchdog()
        mp.msg.warn("thumbfast: watchdog aguardando uma nova sessão de prévias")
        return
    end

    watchdog_retries = watchdog_retries + 1
    local target_time = last_seek_time
    mp.msg.warn("thumbfast: reiniciando gerador de prévias (" .. tostring(reason) .. ")")

    stop_watchdog()
    seek_timer:kill()
    file_timer:kill()
    stop_thumbnailer(true)
    real_w, real_h = nil, nil
    remove_thumbnail_files()

    if restart_timer then restart_timer:kill() end
    restart_timer = mp.add_timeout(0.20, function()
        restart_timer = nil
        if disabled or not show_thumbnail then return end
        spawn(target_time)
        request_seek()
...
local function check_new_thumb()
    ...
    if w then -- only accept valid thumbnails
        ...
        -- A valid newly-written frame proves that the helper and its IPC seek
        -- path are responsive again.
        stop_watchdog()
        watchdog_retries = 0
...
local function clear()
    ...
    watchdog_retries = 0
    if options.quit_after_inactivity > 0 and spawned then
        if show_thumbnail or activity_timer:is_enabled() then
            activity_timer:kill()
        end
        activity_timer:resume()
...
local function thumb(time, r_x, r_y, script)
    ...
    local resume_after_idle = spawned and last_request_clock ~= nil and
        options.restart_after_idle > 0 and
        (now - last_request_clock) >= options.restart_after_idle
    last_request_clock = now
    ...
    if time == last_seek_time and not resume_after_idle then return end
    last_seek_time = time

    -- Even if the normal inactivity shutdown was delayed by the OSC state,
    -- never reuse an old helper after a long period without preview requests.
    if resume_after_idle then
        restart_thumbnailer("retomada após inatividade")
        return
    end
