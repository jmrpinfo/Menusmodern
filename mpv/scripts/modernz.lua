...
local user_opts = {
    ...
    info_mbtn_left_command = "script-binding modern_stats/toggle",
...
local function run_button_command(element_name, command)
    local menu_name = command:match("^script%-binding%s+modernz_menus/([%w_%-]+)%s*$")
    if menu_name then
        local element = elements[element_name]
        local seekbar = elements["seekbar"]
        if element and element.layout and element.layout.geometry and
            seekbar and seekbar.layout and seekbar.layout.geometry then
            local anchor_x = element.layout.geometry.x
            -- Text elements are wide and left-anchored; use the actual click
            -- position for those. Buttons use their stable visual centre.
            if element.type ~= "button" then
                local mouse_x = get_virt_mouse_pos()
                if mouse_x and mouse_x >= 0 then anchor_x = mouse_x end
            end
            mp.commandv(
                "script-message-to", "modernz_menus", "open-at", menu_name,
                tostring(anchor_x), tostring(seekbar.layout.geometry.y),
                tostring(osc_param.playresx), tostring(osc_param.playresy)
            )
            return
        end
    end
...
local function bind_buttons(element_name, use_down)
    ...
    for _, button in ipairs({"mbtn_left", "mbtn_right"}) do
        ...
        if command ~= nil and command ~= "" and command ~= "ignore" then
            elements[element_name].eventresponder[button .. ev] = function() run_button_command(element_name, command) end
        end
    end
    local command = user_opts[element_name .. "_mbtn_mid_command"]
    if command ~= nil and command ~= "" and command ~= "ignore" then
        elements[element_name].eventresponder["shift+mbtn_left_down"] = function() run_button_command(element_name, command) end
    end
    for _, button in ipairs({"wheel_up", "wheel_down"}) do
        local command = user_opts[element_name .. "_" .. button .. "_command"]
        if command ~= nil and command ~= "" and command ~= "ignore" then
            elements[element_name].eventresponder[button .. "_press"] = function() run_button_command(element_name, command) end
        end
    end
end
...
local function hide_osc()
    -- Keep the controls visible while the attached ModernZ popup is open.
    -- This also prevents deadzone_hide=instant from detaching the popup from
    -- its launcher as the pointer moves upward into the menu.
    if state.modernz_menu_open then
...
mp.register_script_message("modernz-menu-state", function(value)
    state.modernz_menu_open = value == "open"
    state.showtime = mp.get_time()
    if state.modernz_menu_open then show_osc() end
    request_tick()
end)
